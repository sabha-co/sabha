...
**Related applications — do not conflate.** This repository is the chat app. **sabha.co** is this app in SaaS mode. **Self-hosted** is this app in default single-tenant mode: **one community per instance**, and operators may run **N instances** on their own servers (each a distinct origin). **[sabha_cloud](https://github.com/sabha-co/sabha_cloud)** is a **separate Rails app** at `cloud.sabha.co`; it provisions dedicated droplets. It is not this codebase and is not a chat origin. Each droplet runs this app in self-hosted mode — from sabha.co and from [sabha-desktop](https://github.com/sabha-co/sabha-desktop) that origin is added by URL like any of the N self-hosted instances.
...
- Keep new SQL PostgreSQL-compatible. SQLite is the default engine, but the app stays Postgres-ready so a future self-hosted Postgres option is a deploy change, not a rewrite (the SaaS untenanted DB already runs Postgres). Prefer Arel/adapter-agnostic ActiveRecord; where raw SQL is unavoidable use `matches` (not a bare `LIKE`), boolean `TRUE`/`FALSE` (not `1`/`0`), a `CASE` clamp (not scalar `MAX`/`GREATEST`), and branch on `connection.adapter_name` only where no common form exists. Full-text search is engine-specific — route index reads/writes through `Message::SearchIndex` (provisioned via `ensure!`, outside `db/schema.rb`), never a migration.
...
- **AnyCable-Go 1.6.9 is the minimum version.** The HTTP API was added in 1.6.9; 1.6.8 and earlier have no `/api` server, so every presence read 404s and push gating silently falls back to the `connected_at` column. `bin/dev` preflights the version and `bin/setup` upgrades an older one, but a hand-rolled deployment gets no such check — confirm `Handle API requests at … (authorization required)` appears in the AnyCable-Go startup log, and watch for `[presence] … unavailable: HTTP 404` in the Rails log.
- **Presence needs a broker.** Every launch point must set `ANYCABLE_BROKER=memory`; without a broker AnyCable-Go reports presence as unsupported and push fails open. This applies to the load harness (`bin/load-anycable`) as much as to production — a harness that drifts from the deploy accessories measures the pre-presence path while looking like it measured the current one. `test/integration/anycable_required_test.rb` guards all four launch points against this drift.
