class Rooms::RefreshesController < ApplicationController
  ...
  def show
    @new_messages = Bookmark.with_bookmark_status(@room.messages.for_display.page_created_since(@last_updated_at))
    @updated_messages = Bookmark.with_bookmark_status(@room.messages.for_display.without(@new_messages).page_updated_since(@last_updated_at))
  end
