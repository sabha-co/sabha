...
|---|---|---|---|
| PR 1 · R0 AnyCable required | U1, U11 | **Complete, in review** | [#138](https://github.com/sabha-co/sabha/pull/138) (`anycable-required`) — one hard cutover, not a deprecate-then-cutover pair |
...
| PR 4 · R3 AnyCable presence | U6 | **Complete, in review** | [#141](https://github.com/sabha-co/sabha/pull/141) (`anycable-presence`, stacked on #140) |
| PR 5 · R4 @everyone guardrail | U7, U8 | **Complete, in review** | [#142](https://github.com/sabha-co/sabha/pull/142) (`everyone-guardrail`, stacked on #141) |
| PR 6 · R5 defer residual | U9 | **Complete, in review** | [#143](https://github.com/sabha-co/sabha/pull/143) (`defer-residual-broadcasts`, stacked on #142) |
| PR 7 · R6 JWT | U10 | **Complete, in review** | [#144](https://github.com/sabha-co/sabha/pull/144) (`jwt-identification`, stacked on #143) |
...
- **2026-07-10** — R4 log-verified in dev (SaaS, workspace `1000006`, room "General" grown to 1067 active members). A capped `@everyone` keeps the durable surfaces — `Notification` bulk insert (Activity rows) + the `increment_unread_notifications_counters` badge bump both fire — while the O(members) fan-out is bounded to zero: no `BroadcastMentionNotificationsJob` and no whole-room `Push::Subscription … user_id IN (…)` resolution (both present in the sub-ceiling 467-member run, absent at 1067). Compose confirm shows "Notify all 1067 people" with the room-wide note. UI refinement: the confirm bar moved above the composer input (rounded bar) with a `bell-alert` icon.
- **2026-07-10** — U7 + U8 implemented on `everyone-guardrail` (R4 ships as one PR, stacked on #141). Ceiling and confirm threshold live in `config/initializers/mentions.rb` (`EVERYONE_MENTION_CEILING`=1000, `EVERYONE_MENTION_CONFIRM_THRESHOLD`=15, both ENV-tunable). Above the ceiling (`Room#everyone_mention_over_ceiling?`), a `Message#everyone_mention_capped?` @everyone keeps the durable `Notification.insert_all` rows + the single badge bump, but `Rooms::Open#applicable_activity_types` drops `:mention` (so the whole-room `:mention` push and missed-email bundle don't fan out), and `create_mention_notifications` / `broadcast_notifications` skip the per-recipient activity-indicator, Activity-append, and live-badge loops — it rides `:everyone_room_message` room-wide instead. The admin/open floor is untouched (AE5 stays pinned in message_test). U8 adds a composer confirm bar: `composer_data_options` exposes the threshold, `active_member_count`, and cap flag; the composer controller shows an inline confirm (never a modal) with the recipient count when @everyone is present and the room is at/above the threshold. Both suites green.
- **2026-07-15** — **KTD4's push half is superseded** by `docs/plans/2026-07-15-001-perf-presence-gated-push-unified-client-plan.md` (U1–U2 landed on `perf-presence-gated-push`). What changed is the reasoning, not just the code. KTD4 held that "presence replaces the real-time driver, not the persisted model," and that push must keep reading `connected?` — sound while nothing could read the Go-side set, but anycable-go's HTTP API (`GET /api/presence/:stream/users`, added in **1.6.9**) makes the live set readable from Rails, so the premise expired rather than being wrong. Push gating now reads that set (`Room::PresenceSet`, subtracted in `Message#watching?`) and `receives_push_for?` no longer consults `connected?` at all. **The persisted column stays** — KTD4's other half holds: email's `workspace_locally_away?`, the activity tiers, and the read/unread scopes still read it, so this is a narrowing of KTD4, not a reversal. `connected?` survives as the fallback when the broker can't answer. The 60s `CONNECTION_TTL` grace window is replaced for push by anycable-go's `--presence_ttl` (45s, now set explicitly rather than inheriting the 15s default, which sat under the client's worst-case reconnect).
...
- **2026-07-10** — U9 log-verified in dev (SaaS, workspace `1000006`). A mention send leaves only two O(1) shared broadcasts on the request (`room_list` nudge + `Rooms::Open:messages` append) — the per-recipient `sidebar_activity_indicator` render/broadcast is gone from the request and now fires inside `BroadcastMentionNotificationsJob` (alongside the `inbox_activity` append), with the live badge push in `BroadcastUnreadNotificationsJob`; both perform tenant-scoped. Plain (mention-less) sends enqueue neither job. Opened as [#143](https://github.com/sabha-co/sabha/pull/143).
- **2026-07-10** — U9 implemented on `defer-residual-broadcasts` (stacked on #142). The two residual synchronous per-recipient loops in `MessagesController#create` move off the request path. The activity-indicator render folds into `BroadcastMentionNotificationsJob` (same recipients, `:user` already preloaded via `with_message_and_creator` — mirrors the Activity-append/indicator pairing in `Notification.delete_all_and_broadcast`), so the synchronous `each(&:broadcast_activity_indicator)` in `create_mention_notifications` is gone. The `UnreadNotificationsChannel` live-badge loop in `broadcast_notifications` defers to a new plain, idempotent `BroadcastUnreadNotificationsJob` (patterned on `BroadcastMentioneeSidebarUpdatesJob`, `message_id:`-keyed, re-resolves recipients in-tenant), guarded so plain messages enqueue nothing. The durable surfaces stay synchronous (the `Notification` rows and the `increment_unread_notifications_counters` bump are O(1) SQL); the capped-`@everyone` path is untouched (both loops already returned early). Both suites green (self-hosted 1700, SaaS 294).
- **2026-07-10** — R0 shipped as a single **hard cutover** in [#138](https://github.com/sabha-co/sabha/pull/138) (`anycable-required`), not the planned deprecate-then-cutover pair. The in-process ActionCable/Redis fallback is removed outright: `any_cable` in every app env, `anycable-go` the dev default (folded into `bin/dev` with a preflight + `bin/setup` install step), the whisper/`AnyCable::Rails.enabled?` gates dropped, and `ANYCABLE_ENABLED=false` now aborts boot with an install hint (a guard test pins the fallback shut). So **U1 (deprecate) and U11 (cutover) both land in #138** — PR 1b is not a separate later release. The AnyCable-only transport units (U5/U6/U10) inherit a fallback-free base, consistent with the earlier "R0 treated as merged for sequencing" decision; the dev `$pubsub`-fallback caveats in older U5/U6 log lines are moot post-cutover.
- **2026-07-10** — U10 implemented on `jwt-identification` (stacked on #143), opened as [#144](https://github.com/sabha-co/sabha/pull/144). anycable-go identifies the socket in Go from a signed JWT carried in the `action-cable-url` meta tag (`?jid=`), skipping the Rails connect RPC on reconnect (`--enforce_jwt` stays off, so the tokenless RPC path still serves bots and first-connect; `Connection#connect` is unchanged). Decisions grounded in the code: **(1) tenant rides as its own plain-string `current_tenant` claim** alongside the `current_user` GID — the gem's `around_command :with_tenant` opens the tenant DB with it *before* the GID is lazily resolved; the GID's embedded `?tenant=` is only a cross-tenant safety assertion (the SaaS test proves a workspace-A token raises `WrongTenantError` under workspace B). **(2) Two-secret split** via `ANYCABLE_JWT_SECRET` (client-facing: signs JWT identity + signed streams) distinct from the RPC/broadcast `ANYCABLE_SECRET`; Rails (`config/anycable.yml`) and every anycable-go launch point fall back to `ANYCABLE_SECRET` when it's unset, so a single-secret deploy is unchanged and moving the U5 stream secret is a no-op until an operator opts in (kept out of the required Kamal secret lists; documented in the deploy configs + env samples). **(3) KTD6 preservation:** the token is minted only from the already-authenticated request (WorkspaceMembership checked, tenant `User` already materialized), and ban/deactivation already force-disconnects via `close_remote_connections` (scoped by `current_tenant`+`current_user` = the JWT identifiers) with `reconnect: false`; `jwt_ttl` (1h, `ANYCABLE_JWT_TTL`-tunable) backstops the misbehaving/native reconnect. **(4) Native-readiness:** `GET /api/cable` returns the cable URL + a fresh token + `expires_in` (session-authed, JSON 401 when unauthenticated) as the web refresh sibling and the native track's token/transport contract — socket credentials only, no content API. Both suites green (self-hosted 1705, SaaS 297).
...
- Wire the web ActionCable consumer to refresh its `jid` token via `GET /api/cable` on a `token_expired` disconnect. Today the stock `@rails/actioncable` consumer reconnects with the same (now-expired) token, so a session held past `jwt_ttl` only recovers on the next full page load — fine at the 1h default, but any move to a short `jwt_ttl` needs this client-side refresh first (belongs with the native track, which uses `@anycable/web`'s `refreshAuth`).
- Measure the establishment-path ceiling under JWT identification on the load-test harness and set `RAILS_MAX_THREADS` (the still-open empirical item for R6/U10); the k6 `chatter.js` harness authenticates via cookie, so exercising the JWT-skips-RPC path needs a token-carrying variant.
