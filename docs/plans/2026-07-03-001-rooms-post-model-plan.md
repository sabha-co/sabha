...
- **Per-reply participant aggregate.** `broadcast_gallery_card` fires on every reply and `participant_creators` runs a full `GROUP BY creator_id` over the post's messages (`thread.rb:69`, moving to `Room::Participants` unchanged) — cost grows with post size, and the same aggregate runs over the page's posts on every gallery load (`preload_participant_creators`). This rework doesn't fix either; both survive verbatim. **Eventual fix:** denormalize the first ~5 distinct participant-creator IDs onto the post (a column/JSON on `rooms`), appended only when a *new* creator posts and the set is under the cap — and skip recompute entirely when the replier is already a participant or the cap is full. One store fixes both the per-reply broadcast and the per-gallery-load preload. Not urgent (a single-post, `room_id`-indexed scan capped at 5) but *unbounded* — do it before forums grow large, hot posts.
...
- **Forum-deactivation cascade.** Deleting a forum soft-deletes every post and its replies in **one synchronous transaction** — `deactivate_threads` iterates each post and runs `update_all` over its memberships + messages (`room.rb:346`, `thread.rb:171`), i.e. O(posts × (members + replies)). D2 shrinks the `× members` term to `× participants` (lazy memberships), but the `× replies` message cascade survives and stays synchronous — a large forum times out the request and holds a long write transaction. **Eventual fix:** a resumable `ForumDeactivationJob` (batch posts, `checkpoint!` via `ActiveJob::Continuable`, idempotent on resume — the `clean_inaccessible_data_later` shape). A `forum_deactivated_at` soft-flag that gates access off the forum's state without touching per-post rows is the deeper alternative, but it broadens every post query and tangles with the `cascade_deactivated`/R15 restore marker — deferred as the bigger-blast-radius option. This rework must also repoint the cascade from the `parent_message_id` join to `Rooms::Post.where(parent_room_id:)` (see the D6 association-cleanup risk).
