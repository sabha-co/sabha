...
**As built (2026-08-12):** five commits — `32b3720` (protective pins: grouping + hover reveal, written first), `f64f7c4` (action bar), `085bb26` (row scale), `8f1426b` (avatars), `cec11ad` (composer + typing row).

- *Action bar:* quick reactions are the first four of `EmojiHelper::REACTIONS` + a boost-input opener + Reply (visible label) + the overflow menu (which keeps the full reaction grid and menu items). Floating treatment is scoped to `.message__meta .message__actions` so the edit form's close-button block and welcome rows behave correctly. The hidden state uses **inherited `visibility`, not opacity alone** — an opacity-only bar stays an invisible hit target over the previous row (and `.btn` re-enables `pointer-events`). Reveal = `.message:is(:hover, :focus-within, :has([open]))`; the row's paint containment (`content-visibility: auto`) clips the floating bar, so the existing `:has([open])` containment lift was extended to `:hover`/`:focus-within`. Touch is unchanged: one-menu model, quick reactions `display: none`.
- *Three CSS traps found:* (1) `.message__action-btn`'s `flex-basis: 25%` (an overflow-menu concern) both mis-sized bar links and broke the bar's `max-content` measurement — neutralised in `.message__bar-btn`, which must sit **after** that rule in the file (equal specificity). (2) buttons.css's circle-button rule (`:has(.for-screen-reader):has(.icon)` → stacked grid) eats any icon+label button — Reply and Send drop the sr-span and carry `aria-label` instead. (3) overrides.css had a stray top-level `}` (removed).
- *Row scale:* author 14.5/700 `--color-text`, time/(edited) 11.5 `--color-text-subtle`, body 15/1.55 `--color-text-lighter`, meta 2px above body, avatar column 36px + 12px gap. Chips: 23px pill, 17px avatar, `boost--mine` accent tint stamped client-side in `boost_delete_controller` (chips are cached per [boost, booster], so "mine" can't render server-side). The redline's aggregated multi-reactor chip was **not** adopted — Sabha boosts are per-user chips; aggregating is a data-model change, not a reskin.
- *Avatars:* `--avatar-border-radius: 25%` (9px at 36px, scales to sidebar 22px / chip 17px); soft 15%-alpha ring replaces the solid black one; forum-card avatars and the DM participants header aligned; inline mention chips deliberately stay pill (text context).
- *Composer:* the typing indicator is a **reserved 22px static row** as the first child of `.composer` (the footer is already outside the scroller) rather than a literal sibling; the provisional thread panel reserves an empty row so materialising the thread doesn't shift the composer. Label grammar ("X is typing" / "X and Y are typing" / "X, Y and N others are typing", first names in company) lives in `TypingTracker.label`, tested in-browser via importmap `import("models/typing_tracker")` — cross-session whisper delivery isn't testable under the `test` cable adapter. Send: visible label + `aria-label`, `name="send"` kept for the test helper, accent fill toggled by `composer--drafting` (new `composer#draftChanged` on `trix-change`, cleared on reset). Card: input on top, tools + Send in a row below; toolbar (when toggled) sits above the input under a hairline, `position: static` overriding the sticky-bottom placement that remains for the edit form.
- *Behaviour audit (redlines list):* already true — own-typing suppression, client-side expiry (5s purge), Send enabled during uploads, unread separator positioned once server-side. Built — focus-within reveal, reserved row, accent Send, typing labels. **Deferred** — optimistic reaction chips (boosts stay round-trip; revisit if latency ever shows), the always-visible slim B/I/S/`</>`+@/# format bar (Trix's toggled toolbar retained, restyled; the custom bar belongs with the parked Lexxy migration), and the search button stays outside the card (spec draws search inside the tools row).
- *Test infra:* `join_room`'s ObsoleteNode retry had never fired — `Capybara::Cuprite::ObsoleteNode` descends from `StandardError`, not `RuntimeError`; the rescue now names `Capybara::Cuprite::Error`. New row tests navigate by sidebar click (same rationale as `message_composer_test`).
- *Verified:* full suite 1831 green, tight wall + row tests 17 green twice, SaaS 305 green, screenshots light + dark (bar, chips, avatars, composer states, thread panel).
