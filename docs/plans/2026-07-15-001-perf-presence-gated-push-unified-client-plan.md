...
- KTD10. **Session restore is gated on a revocation verification covering room-level kicks, not just bans.** Restored sessions skip `Connection#connect` and channel re-authorization, so every revocation path depends on the remote disconnect purging the broker's session cache. That cache is already live server-side — anycable-go's `--sessions_ttl` (default 300s) governs it with the memory broker, and the `restore_from_cache`/`cache_ttl` keys in `config/anycable.yml` are inert (see U1). Only the stock client's silence keeps restore unused today, so this gate is what stands between a kick and a rebuilt subscription the moment U5's client can present a `sid`. Room-membership revocation is the sharper case, not ban: destroying or deactivating a membership calls `close_remote_connections(reconnect: true)`, which actively invites the client back — under restore it would rebuild the exact subscriptions the kick was meant to drop, without re-running `subscribed`. Ban uses `reconnect: false` and is bounded by `jwt_ttl`. Verify purge against the kick path before enabling restore; if it doesn't hold, ship Track 2 without restore — history replay and jittered reconnect land regardless.
...
```mermaid
flowchart TB
  Cable["@anycable/web cable (ext protocol)\none socket · history · restore · refreshAuth→GET /cable"]
...
- R12. Ban, deactivation, or kick terminates stream delivery within the enforcement window today's behavior provides, including for restored sessions.
...
- **Test scenarios:**
  ...
  - No double-present: a fresh connect and an ordinary resubscribe each increment `connections` exactly once (server-side `on_subscribe :present` only); the client adds `present` on tab-visible and on `restored: true`, never on the subscription's `connected` event.
...
- **Verification:** Log-verified dev runs (network tab: one socket; anycable-go logs: ext protocol negotiated); both test suites green (server-side contracts unchanged).
...
### U7. Reconnect recovery: history replay, catch-up wiring, restore gate

- **Goal:** Reconnects recover missed broadcasts exactly once, and session restore only ships if revocation enforcement survives it.
- **Requirements:** R10, R12
- **Dependencies:** U5
- **Files:** `app/javascript/lib/cable.js`, `app/javascript/controllers/refresh_room_controller.js`, `app/javascript/controllers/presence_controller.js`, `saas/test/` and `test/channels/` (enforcement pins where testable server-side)
- **Approach:** Enable history requests on reconnect for the message and state streams; on `reject_history`/overflow, trigger the existing `refresh_room` HTTP catch-up (KTD11). Verify replay side effects: Turbo appends dedupe by DOM id but must not replay sounds, resurrect unread styling, or duplicate `broadcast_reactivation`'s `after`/`prepend` nodes; check whether whispers enter broker history (stale typing flashes must be absorbed by the tracker's expiry). Then the restore gate (KTD10): exercise the kick path first — it disconnects with `reconnect: true`, so the client comes straight back and is the strictest test of whether the broker purged the session — then the ban path. Verify the old `sid` is refused and a fresh subscribe re-authorizes. If purge holds, enable restore and send `present` on the `restored` event — restore re-establishes subscriptions without re-running `subscribed`, so nothing else rejoins the presence set (KTD5). If not, ship with restore disabled and record the finding.
- **Test scenarios:**
  ...
  - Covers R12: banned user's restored session receives no further room broadcasts and is absent from the presence set (log-verified; server-side pin where the test adapter allows).
  - Covers R12: member kicked from one room (membership destroyed → `close_remote_connections(reconnect: true)`) reconnects and restores → receives no further broadcasts for that room while keeping its other rooms. Run this in the SaaS suite too, against a tenanted room `Membership` (the kick path is shared, not SaaS-specific) while the user keeps their `WorkspaceMembership` — removing the `WorkspaceMembership` instead exercises `User#deactivate`'s `reconnect: false`, which is the ban path, not this one.
  - Restored session (tab visible) re-presents within seconds: `connected_at` fresh, presence set repopulated.
...
- **Verification:** Log-verified dev matrix: kill socket (blip), restart anycable-go, expire JWT, kick from one room, ban mid-session — each recovers per the matrix above; the restore go/no-go decision is recorded in this plan's log by dated entry.
...
- OQ1. Does `close_remote_connections` purge anycable-go's session-restore cache — for the kick path (`reconnect: true`) as well as ban? Execution-time verification in U7; gates enabling restore (KTD10). Fallback is shipping without restore.
...
- OQ3. Resolved: `@anycable/core`'s ActionCable channel maps every connect — fresh, reconnect, and restore alike — to `notify('connected')` with no arguments, so there is no per-subscription `restored:` callback. The `present` hook attaches to `connected:` and covers all three; anything restore-specific listens at the cable level via `cable.on('connect', ({ restored }) => …)`.
- OQ4. How does `connections` recover from a broker restart, once `connected?` depends on it and `disconnect_all` is gone (KTD4/KTD6)? Blocks U3. Candidates: derive the count from the presence set on `present`; clamp it on `present` against what the broker reports; or zero it per tenant on a restart signal. Freshness cannot bound it — pick a mechanism that doesn't rely on the timestamp.
- OQ5. What should a status dot show after a member fully disconnects? Preserving `connected_at` (KTD4) means `activity_status` reads `:active` for 10 minutes where today the nil-ing flips it to `:offline` at once — visible on the member directory dot, its sort order, and the online count. Options: accept the tier semantics (last-seen-based, like the away tier), or make `activity_status` connections-aware so dots stay immediate while last-seen stays truthful for email. Decide before U3; it is a product-visible change either way.
- OQ6. Is the heartbeat's steady-state write cost worth measuring against the establishment ceiling before U1–U4 land? The harness reaches ~3,400 connections, where the heartbeat is ~136 writes/s — a baseline run would tie R4's write reduction to a capacity outcome instead of leaving it a write count. If the answer is no, the Problem Frame's headroom framing stands on its own and R4 stays a write-reduction claim.
...
- **Compat-layer behavioral gaps** (`connected`/`restored` mapping, ActionCable internals like `connection.monitor`): audited in U5; the k6 harness and log-verified runs catch regressions.
