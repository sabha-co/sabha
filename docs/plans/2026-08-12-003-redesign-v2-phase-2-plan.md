...
**Status:** in progress — WS0, WS1, WS2, WS4, WS6 built (2026-08-12); WS3 scrim dialogs built (2026-08-13); WS3 emoji button + WS5 Retry still flagged · **Date:** 2026-08-12 · **Builds on:** Phase 1 (`redesign-v2`, Steps 1–6, unpushed) · **Source of truth:** `~/dev/design_handoff_sabha_v2` (design references, not code)
...
**WS5 Message Retry → flagged (idempotency decision needed).** The `.message--failed` treatment already exists; the missing piece is the Retry action. But `Message#to_key = [client_message_id]` and there is **no unique index** on `client_message_id`, so a naive re-POST can create a duplicate on the "succeeded-but-response-lost" edge, and reconstructing the exact create request from the optimistic node is fragile. A robust Retry wants a server idempotency guard (a unique index + upsert-on-`client_message_id`) plus a clean re-drive — a bigger change than a reskin. Recommend deciding the idempotency approach before building.
