...
> **Scope (owner call, 2026-08-12): ①②③ + data-driven ④.** In: the type system; responsive completion; the contextual right-panel; **a reactions rebuild** (reskin + grouped one-chip-per-emoji aggregation shipped — only the optimistic insert is deferred); v2 scrim dialogs; the inbox/activity **data** surfaces; loading skeletons; the live connection banner + message Retry; the six-box OTP input; auth polish. **Not** in: net-new features.

## Confirmed directions (owner call, 2026-08-12)

| # | Fork | Decision | Effect |
|---|---|---|---|
| **P2-1** | **Scope depth** — visual-only vs. + data/behavior vs. + features | **+ data-driven UX (④ minus features)** | Adds Activity verb rows, Threads-inbox cards, connection banner, and Retry on top of the visual/DOM rebuilds. Reactions were reskinned and rebuilt to grouped one-chip-per-emoji aggregation; only the optimistic insert stays deferred. Net-new features stay out. |
| **P2-2** | **Composer format bar** — the persistent B/I/S/`</>`\|@/# bar is entangled with the parked Trix→Lexxy migration | **Defer to Lexxy** (since shipped) | The Trix→Lexxy migration landed, so the always-on format bar shipped as Lexxy's toolbar (with a grafted `@` insert; `#` intentionally omitted — see `docs/redesign/handoff-divergences.md`), alongside the rest of the composer polish (emoji button, search-in-tools). |
| **P2-3** | **Reaction chips** — per-user (today) vs handoff's aggregated | **Reskin now; rebuild later** (owner call, 2026-08-12 — the rebuild since shipped) | The chip styling was moved to the handoff, and the true one-chip-per-emoji aggregation (a boost render/broadcast change) subsequently shipped. Only the optimistic (client-inserts-before-broadcast) behavior is deferred to a future functional pass. |
...
| **WS3** Reactions | ◐ Partial | `28ed6f9` `3015d05` | Reactions reskin was **already done in Phase 1**, and the grouped one-chip-per-emoji **aggregation rebuild subsequently shipped** (`d4a2f73` `c4e8cc1` `d877f6c` `d1afae4`) — only the optimistic insert stays deferred; **scrim dialogs built 2026-08-13** (reaction picker + role menu on a new native-`<dialog>` primitive). The composer emoji button + format bar shipped with the Lexxy migration; move-search shipped 2026-08-13 — see WS3 notes below. |
...
| **WS5** States | ◐ Partial | `11bd15f` `2e5259f` `84bcf2d` `4b04b91` | Live connection-lost banner, sidebar cold-start skeleton, and Message Retry (`84bcf2d` server, `4b04b91` client) built. The remaining skeletons are flagged — see WS5 notes below. |
...
- **Reactions reskin → already built in Phase 1; aggregation rebuild since shipped.** `boosts.css` already carries the v2 look (23px pill, radius 999, 1px border, 17px reactor avatar, `boost--mine` accent tint via `boost_toggle_controller`, trailing add affordance). The comp's extra nuance — reactor avatars "overlapping −5" — is the *aggregated* one-chip-per-emoji look, which P2-3 first deferred and which subsequently **shipped** (`d4a2f73` `c4e8cc1` `d877f6c` `d1afae4`): `_boosts.html.erb` renders one chip per distinct reaction via `message.boost_groups`, `_boost.html.erb` stacks reactor avatars with a `+N` overflow capped at `Boost::AVATARS_SHOWN` (5), and `boost.rb` does a whole-container `broadcast_boosts` replace. Only the optimistic insert stays deferred.
- **Composer emoji button → ✅ shipped with the Lexxy migration.** `app/views/rooms/_composer_fields.html.erb` carries a working `composer__emoji-btn` opening a scrim-dialog emoji grid (`composer#insertEmoji`), alongside Lexxy's always-on format bar. (Superseded the earlier "net-new emoji picker needs owner sign-off" concern.)
...
> **◐ Partial.** Reactions reskin was already done in Phase 1, and the grouped one-chip-per-emoji aggregation rebuild has since shipped (only the optimistic insert stays deferred). Scrim dialogs shipped as a rebuild with its own tested pass; the composer emoji button + format bar shipped with the Lexxy migration; move-search shipped 2026-08-13 — see Build status.

- **Reactions reskin (RESKIN, planned — the pipeline was later reshaped):** the plan was to keep **per-user** chips and leave the boost pipeline untouched (`_boosts.html.erb`, `messages/boosts/_boost.html.erb`, `Boost#broadcast_boosts`, `create.turbo_stream.erb`), moving only the chip *styling* closer to the handoff — pill shape, ~17px booster avatar, accent tint on your own chip (the `boost--mine` class is stamped by `boost_toggle_controller`), and a trailing `＋` add affordance beside the row. **Update:** the true one-chip-per-emoji aggregation rebuild subsequently **shipped** — the pipeline no longer keeps one chip per reactor; `_boosts.html.erb` renders one chip per distinct reaction via `message.boost_groups` and `boost.rb` does a whole-container `broadcast_boosts` replace. Only the optimistic insert stays **deferred** (see Out of scope).
...
- **Composer polish (RESKIN):** add the emoji-picker button to the tools row; move Search **inside** the tools row (`app/views/rooms/show/_composer.html.erb:15-18`). **Format bar since shipped:** the Trix→Lexxy migration landed, so the always-on format bar shipped as Lexxy's toolbar (with a grafted `@` insert; `#` intentionally omitted — see `docs/redesign/handoff-divergences.md`), and the emoji button + markdown input are live (P2-2).
...
> **◐ Partial.** Built: the live connection-lost banner (`11bd15f`), the sidebar cold-start skeleton (`2e5259f`), and Message Retry (`84bcf2d` server, `4b04b91` client — the `client_message_id` idempotency approach is recorded in Build status). The room first-paint skeleton has no slot; the older-messages spinner already exists.
...
- **Designer boundary check:** no routes/models/controllers/Turbo-target/partial-structure change **except** the sanctioned ④ data item — the activity/thread queries (WS4). WS3 was later widened past reskin-only: the boost model/pipeline was reshaped for grouped one-chip-per-emoji aggregation (`broadcast_boosts` whole-container replace) — an accepted divergence, with only the optimistic insert still deferred.
...
- **Optimistic reaction insert** — the grouped one-chip-per-emoji aggregation rebuild **shipped**: the boost pipeline now renders per-emoji-group chips and does a whole-container `broadcast_boosts` replace. What remains out of scope is the *optimistic* (client-inserts-before-broadcast) behaviour — the insert would need reconciling on `(booster_id, content)` since boosts carry no `client_message_id`. Recorded for a future functional pass.
...
- ~~**Composer persistent format bar** — until the Trix→Lexxy migration lands (P2-2).~~ **Shipped:** the migration landed and the always-on format bar shipped as Lexxy's toolbar (with a grafted `@` insert; `#` intentionally omitted — see `docs/redesign/handoff-divergences.md`).
