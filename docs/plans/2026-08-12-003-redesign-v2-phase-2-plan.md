...
- **Scrim dialogs → flagged (rebuild, its own tested pass).** No dialog/scrim infra exists today; the "reaction picker" and message-options popups are a working, tested `<details>`/`popup` flow (`_actions.html.erb`) carrying Reply/Quote/Edit/Delete. Converting to `<dialog>` scrim changes Turbo/focus mechanics on that flow — a real rebuild that deserves a focused, tests-first pass rather than a rushed one, since a regression breaks core message actions.
