---
title: Your workspaces in one place — sabha.co as the switcher for self-hosted Sabha
type: feat
status: ready
date: 2026-09-23
...
**A self-hosted admin.**
> "I want my members to find us easily and sign in without friction, but my workspace keeps its own accounts. Nobody outside decides who gets in."
...
Your list sits in the workspace selector, mixed with your sabha.co workspaces, in the order you choose. A self-hosted entry is a link to that workspace's own site. If you've signed in there before in this browser, you go straight in. If not, you see its login page, with the sabha.co shortcut when it has one.
...
The desktop app (Electron) and the mobile app (Hotwire Native) both show each workspace's own web UI, so they work the same way. Sign in to sabha.co in the app once and your whole list arrives. Each workspace then needs its own session in the app, obtained in one of two ways:

- **Workspace with the shortcut:** one tap or click. A system browser sheet opens briefly, sabha.co confirms it's you (you already approved Acme once, so there's no screen to answer), and you're handed back to the app. If you aren't signed in to sabha.co in that browser, you sign in there first.
- **Workspace without it:** its own login, shown inside the app, once per device.

Because the app holds a session with every workspace, it's a **real switcher**: unread badges, notifications from all of them, and instant switching. The web can't do this; the apps can. On the phone, notifications also need the push relay (see Not in scope).

### A new laptop or phone

Your list is already there. Workspaces with the shortcut need one tap each; others need one login each. Nothing about your other devices changes.

### Leaving or tidying up

- **Hide** a workspace from the list: it stays hidden until you add it again or sign in with the shortcut.
- **Can't reach it?** If a workspace is down or moved, its entry is marked as unreachable, and it keeps its last known name until it comes back or you remove it.
...
- **Settings → Your Workspaces** is one list, in the selector's order: sabha.co and self-hosted workspaces together, each self-hosted row tagged with its address and status, its actions in a ⋯ menu.

## How it works for an admin

**Doing nothing is fine.** Members can already add your workspace to their sabha.co list. Your workspace's login, members and data stay yours. sabha.co only reads your public name and logo.

**Connecting to sabha.co (optional, one-time)** adds the "Continue with sabha.co" shortcut:

1. On sabha.co, add your workspace from the selector's **+** and tick **I run this workspace**. sabha.co checks it's a Sabha server and gives you a secret, shown once.
2. Add the secret to your workspace's server environment (`SABHA_HUB_SECRET`) and restart.
...
- **You still decide who gets in.** Someone new needs your invite link, even with the shortcut.
...
- Signing out of your workspace signs them out of your workspace only.
...
A workspace that uses its own single sign-on (`AUTH_METHOD=sso`) keeps that as its only way in; the shortcut isn't offered.
...
1. **Your list is yours.** Only you add to it: by pasting an address, by accepting a prompt a workspace shows you, by approving the shortcut, or by owning a Sabha Cloud droplet. A workspace can't add itself to your list.
2. **Sessions stay on your device.** sabha.co never stores a login for another workspace. A new device signs in to each workspace on its own. This is the line we don't cross: a sabha.co that held keys to every workspace would be one breach away from exposing all of them.
3. **Workspaces own their accounts.** The shortcut is a convenience the admin chooses. It never replaces their login, their invite rules, or their sign-out.
4. **Ask before sharing who you are.** sabha.co tells a workspace your name and email only after you approve it, once per workspace.
5. **Never match by email.** Accounts are linked only when their owner, signed in, asks for it.

## What's in scope

**The list (sabha.co)**
- L1. A per-person list of self-hosted workspaces, shown in the workspace selector alongside sabha.co workspaces, reorderable across both, not counted toward any sabha.co workspace membership limit (such as the one proposed in #193). Up to 100 entries per person, with a per-hour limit on adds.
- L2. Add by pasting an address. sabha.co checks it's a Sabha server via its public manifest and shows the workspace's own name and logo, refreshed in the background rather than copied once. Workspaces too old to publish a manifest get a clear "needs to update Sabha" message.
- L3. Add from a workspace: any self-hosted Sabha can show "Add to your sabha.co list" after sign-in. It's a link to sabha.co carrying the workspace's address; sabha.co asks the person to confirm. Members can dismiss it for good; it isn't shown to members who arrived through the shortcut (they're already listed), and the admin can switch it off.
- L4a. Every entry shows the workspace's address under its name, and the add and consent screens show it prominently, so a look-alike name can't pass as another workspace.
- L4b. Logos are copied to sabha.co and served from there, so a workspace never sees who is viewing its logo in someone's list.
- L4c. A workspace that can't be reached is marked unreachable and keeps its last known name.
- L4d. Names are display only and may repeat: a self-hosted "Rust Club" and a sabha.co "Rust Club" are different entries, told apart by the address under the self-hosted one. A self-hosted workspace is identified by its normalised address; pasting `HTTPS://Chat.RustClub.org:443/` finds the same entry as `https://chat.rustclub.org`.
- L4e. Pasting sabha.co itself is refused; pasting a sabha.co workspace address (`sabha.co/1000121`) goes to that workspace's normal join flow instead of the remote list. Sabha Cloud droplets on `*.sabha.co` subdomains are self-hosted workspaces and can be added.
- L4f. If the pasted address differs from the address the workspace calls itself, the add screen says so ("this workspace calls itself chat.rustclub.org") and offers to add that one instead. Both stay allowed; sabha.co never merges two addresses on its own.
...
- S2. The button on the workspace's sign-in and invite pages, alongside its own login.
- S3. A consent screen on sabha.co the first time, per person and workspace.
- S4. The workspace's own rules still apply: invite-only by default, no email matching, linking only from a signed-in profile with a recent password or email-code check, and no removing a member's last way to sign in.
...
- A2. Per-workspace sign-in: the shortcut through the system browser where available (on mobile, `ASWebAuthenticationSession` / Custom Tabs), otherwise the workspace's own login inside the app. Both end in a **cookie** session for that workspace's origin: an Electron partition on desktop, the web view cookie store on mobile (#194's PKCE hand-off and session claim).
- A3. Desktop: unread badges and notifications per workspace from each workspace's `DesktopChannel` (#194); the workspace's admin must have `DESKTOP_NOTIFICATIONS_ENABLED=true`, which is off by default.
- A4. Mobile: the in-page workspace rail and WebPush bell are hidden for `hotwire_native_app?`, as #194 does for the desktop app, since the native switcher replaces them. Every workspace in the list opens inside the app, in its own `Navigator` (Hotwire Native otherwise opens other domains in an external browser). Unread badges come from the catalog on foreground; push notifications wait for the push relay.
- A5. Mobile path configuration (iOS and Android) is served by sabha.co and applies to every workspace, since all Sabha servers share routes.

## Not in scope

- Showing another workspace's pages inside sabha.co on the web. Entries are links.
- Unread badges in the web list. Only the apps can show them.
- sabha.co storing sessions or tokens for other workspaces, on any device.
- Workspaces sending their member lists to sabha.co. sabha.co only knows what each person adds.
- Changes to custom single sign-on (`AUTH_METHOD=sso`).
- Syncing profiles beyond name and avatar hints when an account is created.
- **Mobile push notifications.** Apple's and Google's push credentials belong to the Sabha app, so a self-hosted workspace can't push to it directly; it needs a push relay run by Sabha, plus a way for each workspace to receive the device token. That relay is its own plan. Until then the mobile app shows badges when it opens.

## Decisions

Settled with the owner on 2026-09-23:

- **The list is the product; the sign-in shortcut is optional.** Any member can add any workspace; pairing speeds up sign-in but isn't required to be listed.
- **Shared secret per paired workspace**, not a Chatto-style public client. It reuses the single sign-on code Sabha already has, and the origin proof closes the pairing gap. The secret-free design stays a possible later swap underneath the same button and screens.
...
- **An admin disconnecting from sabha.co keeps members' entries.** Each entry falls back to a plain one (no shortcut, the workspace's own login); only the stored consent is dropped.
...
13. **The apps get the list through the destination catalog.** `api/destinations/show.json.jbuilder` in SaaS mode adds `self_hosted_peers` after the workspace peers: origin, name, sabha.co's logo endpoint URL, and whether the shortcut is on. A top-level `order` lists every peer id, sabha.co and self-hosted workspaces together, in the person's selector order, so a client can rebuild the mixed order. They read only untenanted rows, so building the catalog never opens a tenant connection pool. They're a separate key so a client that only understands workspace peers keeps working within protocol major 1. For the shortcut, the app opens the workspace's `/session/hub` in the system browser with #194's hand-off parameters, including the PKCE challenge; the workspace issues the session claim and the app redeems it with its verifier. The app holds a cookie session per workspace partition; sabha.co never sees it.
...
15. **Untenanted models and account deletion.** `RemoteWorkspace`, `RemoteWorkspaceMembership` and `RemoteWorkspacePairingRequest` inherit from `UntenantedRecord`, with migrations in `saas/db/untenanted_migrate/`. Deleting a `GlobalIdentity` destroys its memberships and pending pairings and nullifies `paired_by` on workspaces it paired; the pairing itself stays active until the workspace disconnects or re-pairs.
...
17. **Identity is the key, never the name.** A sabha.co workspace is identified by `Workspace.external_id`; a self-hosted workspace by its canonical origin, unique on `remote_workspaces.origin` (a self-hosted install has exactly one workspace, so one origin is one workspace). Memberships are unique per identity and workspace in their own tables; the selector's typed ids (`workspace:<external_id>`, `remote:<id>`) and the catalog's separate `peers` / `self_hosted_peers` keys keep the two id spaces apart, and the desktop app keys sessions by origin (every tenanted workspace shares sabha.co's).
...
19. **Aliases are surfaced, not merged.** The manifest's `workspace.url` is a hint, not proof: any server can claim any address, and an unpaired one can prove nothing. When it differs from the pasted origin, sabha.co shows both and lets the person choose; it ignores hints that aren't public `https` origins (such as an unset `APP_HOST` reporting `localhost`). Pairing and the shortcut stay bound to the exact origin that published `hub_proof`.

20. **Mobile on Hotwire Native.** Hotwire Native renders each workspace's server HTML, so no JSON API is needed for screens; the web app already has a start (`@hotwired/hotwire-native-bridge` pinned, a `bridge--button` component, the `native` stylesheet behind `hotwire_native_app?`, and `GET /configurations/ios_v1`). What the multi-workspace app adds:
    - **One `Navigator` per workspace**, and a custom route decision handler that treats every origin in the person's list as in-app; anything else still opens externally.
    - **Path configuration from sabha.co.** Hotwire Native loads one configuration per app, not per server. sabha.co serves `ios_v1` and a new `android_v1`; rules stay generic because self-hosted workspaces run older versions. A workspace's own `/configurations/ios_v1` is no longer what the app reads.
    ...
    - **Fallback**: Flutter is the backup choice for mobile. If it's ever needed, the list, catalog and consent carry over unchanged; only per-workspace sign-in changes, because a Flutter app would hold API tokens rather than web view cookies.
## Data model
...
remote_workspaces
  ...
  pairing_status (none | active | disconnected, default none),
  paired_via (self_serve | sabha_cloud, nullable),
  paired_by_id (fk to global_identities, nullable), paired_at, last_signed_in_at,
  created_at, updated_at

remote_workspace_pairing_requests            -- pending requests only
...
remote_workspace_memberships
  ...
  consented_at (nullable), created_at, updated_at
  unique [global_identity_id, remote_workspace_id]
```

**Each self-hosted workspace (its own SQLite)**
...
**Workspace (instance)**
- Manifest (`GET /api/manifest`): adds `workspace.name`, `workspace.logo_url`, `workspace.url` (canonical, from `APP_HOST`), and `hub_proof` when paired.
...
- Env: `SABHA_HUB_SECRET`, plus a setting to hide the list prompt. The Hub is always `https://sabha.co` (`Sabha::HUB_URL`), not configurable.
...
- The selector's **+** opens a menu: create a workspace, or add a self-hosted workspace. The add confirmation carries an "I run this workspace" checkbox that starts pairing on the same submit.
- Settings → Your Workspaces: one list across both kinds. A self-hosted row's ⋯ menu holds hide, remove, stop signing in with sabha.co and, for admins, set up, new secret and disconnect; a waiting pairing shows Verify on the row. `resources :remote_workspace_pairing_requests, only: %i[ create show update ]` for pairing requests and Verify; `DELETE /remote_workspaces/:id/pairing` ends the active pairing.
- The consent screen in the provider flow.
- `POST /api/platform/remote_workspaces`: control-plane create (bearer token: HMAC-SHA256 of `SSO_CLOUD_SABHA_SECRET` with `sabha-platform-api`). Creates an already-active pairing, returns origin and secret, and adds the owner's membership when `owner_email` is given.
- Destination catalog: list entries as external peers.

## Verification

- Workspace: `bin/rails test test/controllers/sso/ test/controllers/sessions_controller_test.rb test/controllers/users_controller_test.rb test/models/single_sign_on_record_test.rb test/models/first_run_test.rb test/controllers/api/ test/controllers/sso/handoff_test.rb`, plus new tests for the migration on a database with blank `issuer` rows, the shortcut (including the PKCE hand-off through `/session/hub`), profile linking, `hub_proof`, and the list prompt (shown, dismissed, hidden for linked users, hidden when the admin turns it off).
- sabha.co: `SAAS=true AUTH_METHOD=otp bin/rails test saas/test/`, plus new tests for origin normalisation (case, port, trailing dot, IDN, path stripped), refusing sabha.co's own host and routing `sabha.co/<id>` to the join flow, the alias hint (shown, ignored when not public https), adding by address (guarded probe failures, workspaces from before #194, a different protocol major, rate limit and 100-entry cap), the prompt flow, hide, remove and reorder across sources, logo copying, the refresh job (unreachable marking, cleanup of unlisted rows), pairing, Verify and replacement by a newer verified pairing, provider lookup (records, env, wildcard, disconnected), consent, `self_hosted_peers` in the catalog, account deletion, links from a workspace page resolving without a tenant prefix, the logo endpoint serving stored bytes, and the recurring job running with no tenant.
- Manual:
  - Add an unpaired local workspace by address and through its prompt; confirm the link opens its own login.
  ...
  - Stop the workspace; confirm the entry turns unreachable after the refresh job and recovers when it's back.
  - In the desktop app, open a shortcut workspace; confirm the system-browser hand-off lands in the app signed in, and that a claim redeemed without the verifier is refused.
  - In the mobile app, switch between a sabha.co workspace and two self-hosted workspaces; confirm listed origins stay in-app, an unlisted link opens externally, and the shortcut's claim ends with the web view signed in.
