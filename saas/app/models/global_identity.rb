...
class GlobalIdentity < UntenantedRecord
  ...
  MAX_WORKSPACES = 1
  MAX_MEMBERSHIPS = 20
  DEFAULT_COMMUNITY_ID = ENV.fetch("DEFAULT_COMMUNITY_ID", "1000101")

  class_attribute :enforce_community_caps, default: true
  ...
  after_create_commit :join_default_community
  ...
  def workspace_limit_reached?
    return false if superadmin? || !enforce_community_caps
  ...
  def self.default_community
    Workspace.active.find_by(external_id: DEFAULT_COMMUNITY_ID)
  end
  ...
  private
    ...
    def join_default_community
      community = self.class.default_community
      return unless community

      join(community.external_id.to_s)
    end
end
