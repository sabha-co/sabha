...
module Saas
  class RemoteWorkspacePairingRequestsControllerTest < ActionDispatch::IntegrationTest
    ...
    test "a new secret for a workspace in the list is shown once, with the steps to finish" do
      post "/remote_workspace_pairing_requests", params: { remote_workspace_id: @acme.id }
      ...
      get "/remote_workspace_pairing_requests/#{pairing.id}"
    ...
    test "only workspaces in the person's list can be paired" do
      assert_raises(ActiveRecord::RecordNotFound) do
        post "/remote_workspace_pairing_requests", params: { remote_workspace_id: remote_workspaces(:club).id }
      end
    end
    ...
    test "verify switches the shortcut on" do
      pairing = @alice.request_remote_workspace_pairing!(@acme)
      stub_manifest(hub_proof: RemoteWorkspace.pairing_proof(pairing.secret, @acme.origin))

      patch "/remote_workspace_pairing_requests/#{pairing.id}"
    ...
    test "verify says what to check when the proof isn't there yet" do
      ...
      assert_redirected_to remote_workspace_pairing_request_path(pairing)
    ...
    test "someone else's request can't be seen or verified" do
      pairing = global_identities(:bob).request_remote_workspace_pairing!(@acme)

      assert_raises(ActiveRecord::RecordNotFound) { get "/remote_workspace_pairing_requests/#{pairing.id}" }
      assert_raises(ActiveRecord::RecordNotFound) { patch "/remote_workspace_pairing_requests/#{pairing.id}" }
    end

    test "whoever paired a workspace can disconnect it" do
      @acme.update!(pairing_status: :active, secret: "secret", paired_by: @alice)

      delete "/remote_workspaces/#{@acme.id}/pairing"
    ...
    test "nobody else can" do
      @acme.update!(pairing_status: :active, secret: "secret", paired_by: global_identities(:bob))

      assert_raises(ActiveRecord::RecordNotFound) { delete "/remote_workspaces/#{@acme.id}/pairing" }
      assert @acme.reload.pairing_active?
    end
    ...
    test "settings offers Verify on a waiting workspace and the pairer's actions on a connected one" do
      ...
      pairing = @alice.request_remote_workspace_pairing!(club)

      get "/settings"

      assert_select "a[href=?]", "/remote_workspace_pairing_requests/#{pairing.id}", text: "Verify"
      assert_select ".list-tag--positive", text: "Continue with sabha.co"
      assert_select "form[action=?]", "/remote_workspaces/#{@acme.id}/pairing", text: "Disconnect from sabha.co"
      assert_select "form[action='/remote_workspace_pairing_requests']", text: "Get a new secret"
      assert_select "form[action='/remote_workspace_pairing_requests']", text: "Set up Continue with sabha.co", count: 0
    end
    ...
    test "settings offers setting up the shortcut on a workspace nobody has paired" do
      ...
      assert_select "form[action='/remote_workspace_pairing_requests'] input[name=remote_workspace_id][value=?]", @acme.id.to_s
      assert_select "form[action='/remote_workspace_pairing_requests']", text: "Set up Continue with sabha.co"
    end

    test "a member can take back their approval from settings" do
      @acme.update!(pairing_status: :active, secret: "secret")
