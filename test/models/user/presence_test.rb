...
class User::PresenceTest < ActiveSupport::TestCase
  ...
  test "defaults to available" do
    assert_equal "available", User.new.presence
  end

  test "rejects an unknown state" do
    assert_raises(ArgumentError) { @user.presence = "on_holiday" }
  end
  ...
  test "own dot ignores liveness, since the viewer is demonstrably present" do
    @user.update! presence: :do_not_disturb, last_active_at: nil
  ...
  # the watermark could age past it while someone was still typing, and they'd go
  # amber mid-sentence.
  test "the write throttle stays inside the active window" do
    assert_operator User::Presence::ACTIVITY_REFRESH_THRESHOLD, :<, User::Presence::ACTIVE_WINDOW
  end
  ...
  test "staying active says nothing" do
    ...
    assert_no_broadcasts "#{Current.account.to_gid_param}:presence" do
      @user.interacted
    end
  ...
  test "changing presence announces it and counts as a sign of life" do
    ...
    assert_broadcasts "#{Current.account.to_gid_param}:presence", 1 do
      @user.change_presence! :do_not_disturb
    end

    assert_equal "do_not_disturb", @user.reload.presence
    assert @user.active_now?, "the chooser would otherwise be broadcast as idle"
  end

  test "an unknown state is refused" do
    assert_raises(ArgumentError) { @user.change_presence! "on_holiday" }
  end
  ...
  test "batch resolution matches the one-at-a-time answer" do
    users(:david).update! presence: :available, last_active_at: 1.minute.ago
    users(:jason).update! presence: :do_not_disturb, last_active_at: 30.minutes.ago
