...
class SsrfProtectionTest < ActiveSupport::TestCase
  ...
  test "blocks IPv4-mapped IPv6 addresses with link-local IPs (AWS metadata via mapped format)" do
  ...
  test "blocks IPv4-compatible IPv6 addresses with link-local IPs (the reported AWS metadata bypass)" do
    stub_dns_resolution("::169.254.169.254")
